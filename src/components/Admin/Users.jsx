...
import { EnvelopeIcon, UserIcon, LockClosedIcon, LockOpenIcon } from '@heroicons/react/24/outline';
...
function Users() {
  ...
  const handleBlockUser = async (userId, currentStatus, userName) => {
    ...
    if (window.confirm(`Are you sure you want to ${action} ${userName}?`)) {
      try {
        await api.patch(`/users/${userId}`, { isBlocked: newStatus });
        toast.warn(`User ${action}ed successfully`);
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredUsers.map((user) => {
                const isAdmin = user.email === 'admin@gmail.com' || user.isAdmin;
